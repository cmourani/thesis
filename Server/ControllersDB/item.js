...
itemController = {
  add: function(body, cb) {
    ...
    conn.query(
      `insert into item (name, user_id, event_id) values ${string}`,
  ...
  addMultiple: function(body, cb) {
    ...
    string.length
      ? conn.query(
          `insert into item (name, event_id) values ${string}`,
          function(err, results) {
            if (err) {
              cb(err, null);
            } else {
              conn.query(
                `select * from item where event_id = ${body.event_id}`,
                function(error, res) {
                  if (error) {
                    cb(error, null);
                  } else {
    ...
    conn.query(`select * from item where id = ${id}`, function(err, results) {
      if (err) {
        ...
        if (JSON.parse(JSON.stringify(results))[0].user_id !== null) {
          conn.query(
            `update item set user_id = NULL where id = ${id}`,
            function(err1, results1) {
              if (err1) {
                cb(err1, null);
              } else {
                conn.query(`select * from item where id = ${id}`, function(
                  err2,
                  results2
                ) {
                  if (err2) {
          ...
          conn.query(
            `update item set user_id = ${user_id} where id = ${id}`,
            function(error, res) {
              if (error) {
                cb(error, null);
              } else {
  ...
  getItemsByUserId: function(user_id, cb) {
    conn.query(`select * from item where user_id = ${user_id}`, function(
      err,
      results
    ) {
      if (err) {
  ...
  getItemsByEventId: function(event_id, cb) {
    conn.query(`select * from item where event_id = ${event_id}`, function(
      err,
      results
    ) {
      if (err) {
  ...
  updateField: function(id, field, newValue, cb) {
    ...
    conn.query(`update item set (${q}) where id = ${id}`, function(
      err,
      results
    ) {
      if (err) {
